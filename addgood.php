...
if($cou=='1'){
	...
	$ans = sql_i($sql,array(null,$_POST['mid'],$_POST['aid'],'1'));
	$ans = array('maps'=>array('取消讚'));
}
elseif($cou=='0'){
	$sql = "DELETE FROM goods WHERE mid=? AND aid=?";
        $ans = sql_i($sql,array($_POST['mid'],$_POST['aid']));
	$ans = array('maps'=>array('讚'));
}
